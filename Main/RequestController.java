...
/**
 * A controller class which allows admin to approve or disapprove any pending
 * requests
 *
 * @author Lenovo
 */
...
//        requestTable.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);
        ...
        System.out.println("requests are " + requests);
...
//                RoomBookingController.checkAvailabilty(RoomData.get(requests.get(i).getRoomNumber()), requests.get(i).getDay() , requests.get(i).getDate(), requests.get(i).getStartTime(), requests.get(i).getEndTime())
            }
